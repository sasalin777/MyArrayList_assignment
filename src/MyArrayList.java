...
public class MyArrayList<E> implements List<E> {
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        if (elementData.length <= length + size) {
            elementData = grow(length + size);
            System.arraycopy(elementData, index, elementData, length + 1, length+size);
            System.arraycopy(a, 0, elementData, index, size);
            size += length;
            return true;
        } else if (elementData.length >= length + size) {
            System.arraycopy(elementData, index, elementData, index + length, length+size);
            size += length;
